...
def ocr_from_email(request):\
    ...
    try:\
        # Mailimport-Helfer (Graph-Session, Attachments, Download)\
        from .views_mailimport import get_ms_session, list_attachments, fetch_attachment_bytes\
    except Exception as e:\
        return HttpResponseServerError("Mailimport-Helfer fehlen: %s" % e)\
    try:\
        # OCR/Text-Extraktion + Heuristiken\
    ...
    try:\
        ...
        if request.GET.get("debug"):\
            return JsonResponse({"mid":mid,"aid":pick.get("id"),"issuer":issuer,"date":(inv_date.isoformat() if hasattr(inv_date,"isoformat") else inv_date),"amount":(str(gross) if gross is not None else None)})\
